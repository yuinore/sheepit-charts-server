...
(async () => {
  fs.readdir('./log/projects/', function (err, files) {
    var fileList = files.filter(function (file) {
      return (
        fs.statSync(`./log/projects/${file}`).isFile() && /.*\.html$/.test(file)
      );
    });
    ...
    for (const filename of fileList) {
      ...
      const data = fs.readFileSync(`./log/projects/${filebasename}.html`);
      ...
      const nodes_users = nodes_tables[0].querySelectorAll(
        'td.table_avatar span',
      );
      const nodes_status = nodes_tables[0].querySelectorAll('td.status');
      const nodes_progresses = nodes_tables[0].querySelectorAll(
      ...
      const statuses = Array.from(
        nodes_status,
        (td) => parseInt(td.textContent.trim()) || 0,
      );
      const progresses = Array.from(
        nodes_progresses,
        (td) =>
          Math.round((1 - eval(td.textContent.trim())) * 10000) / 100 || 0,
      );
      const numes = Array.from(
        nodes_progresses,
        (td) => parseInt(td.textContent.trim().split('/')[0]) || 0,
      );
      const denos = Array.from(
        nodes_progresses,
        (td) => parseInt(td.textContent.trim().split('/')[1]) || 0,
      );
